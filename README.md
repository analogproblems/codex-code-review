...
- Session-start policy and a compact per-prompt reminder tell Claude to use `code-review:reviewer` for all code reviews. Redirect notices are short, and reviewer subagents receive a self-contained forwarding contract without repeating the session policy. Opulent's architect, coding, mechanical, test and merge-decision lanes remain unchanged; separate documentation/runbook passes are permitted. This context-size change is a separate commit: reverting it restores repeated policy without reverting verification, reporting or task labels.
