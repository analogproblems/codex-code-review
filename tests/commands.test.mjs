...
const PLUGIN_ROOT = path.join(ROOT, "plugins", "code-review");
...
test("drop-in code-review command invokes the safe pull request reviewer", () => {
  const source = read("commands/code-review.md");
  assert.match(source, /disable-model-invocation:\s*false/);
  assert.match(source, /\[<pr-number-or-url>\] \[--wait\|--background\] \[--force\]/);
  assert.match(source, /pr-review "\$ARGUMENTS"/);
  assert.match(source, /run_in_background:\s*true/);
  assert.match(source, /Preserve the user's arguments exactly/i);
  assert.match(source, /private marker/i);
  assert.match(source, /Do not fix code or apply patches/i);
});
...
test("adversarial review command uses AskUserQuestion and background Bash while staying review-only", () => {
  ...
  assert.match(source, /When in doubt, run the review/i);
  assert.match(source, /\(Recommended\)/);
  assert.match(source, /uses the same review target selection as `\/code-review:review`/i);
...
test("rescue command absorbs continue semantics", () => {
  ...
  // request to the `code-review:codex-rescue` subagent") while running under
  ...
  assert.match(rescue, /subagent_type: "code-review:codex-rescue"/);
  assert.match(rescue, /do not call `Skill\(code-review:codex-rescue\)`/i);
  ...
  assert.match(rescue, /run the `code-review:codex-rescue` subagent in the background/i);
  ...
  assert.match(readme, /`code-review:codex-rescue` subagent/i);
  assert.match(readme, /if you do not pass `--model` or `--effort`, Codex chooses its own defaults/i);
  assert.match(readme, /--model gpt-5\.4-mini --effort medium/i);
  assert.match(readme, /`spark`, the plugin maps that to `gpt-5\.3-codex-spark`/i);
  assert.match(readme, /continue a previous Codex task/i);
  assert.match(readme, /### `\/code-review:setup`/);
  assert.match(readme, /### `\/code-review:review`/);
  assert.match(readme, /### `\/code-review:adversarial-review`/);
  assert.match(readme, /uses the same review target selection as `\/code-review:review`/i);
  assert.match(readme, /--base main challenge whether this was the right caching and retry design/);
  assert.match(readme, /### `\/code-review:rescue`/);
  assert.match(readme, /### `\/code-review:transfer`/);
  assert.match(readme, /### `\/code-review:status`/);
  assert.match(readme, /### `\/code-review:result`/);
  assert.match(readme, /### `\/code-review:cancel`/);
});
...
test("setup command can offer Codex install and still points users to codex login", () => {
  ...
  assert.match(readme, /\/code-review:setup --enable-review-gate/);
  assert.match(readme, /\/code-review:setup --disable-review-gate/);
});
