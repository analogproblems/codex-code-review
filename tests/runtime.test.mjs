...
import { loadBrokerSession, saveBrokerSession } from "../plugins/code-review/scripts/lib/broker-lifecycle.mjs";
import { resolveStateDir } from "../plugins/code-review/scripts/lib/state.mjs";
...
test("transfer delegates the current Claude session directly to native import", () => {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      { type: "user", cwd: repo, message: { role: "user", content: "/code-review:transfer" } }
  ...
  assert.deepEqual(
    ...
    ["Initial request", "Initial answer", "/code-review:transfer"]
  );
});
...
test("review rejects focus text because it is native-review only", () => {
  ...
  assert.match(result.stderr, /\/code-review:adversarial-review focus on auth/i);
});
...
test("status shows phases, hints, and the latest finished job", () => {
  ...
  assert.match(result.stdout, /`\/code-review:status review-live`<br>`\/code-review:cancel review-live`/);
...
test("stop hook logs running tasks to stderr without blocking when the review gate is disabled", () => {
  ...
  assert.match(blocked.stderr, /\/code-review:status/i);
  assert.match(blocked.stderr, /\/code-review:cancel task-live/i);
});
...
test("stop hook does not block when Codex is unavailable even if the review gate is enabled", () => {
  ...
  assert.match(allowed.stderr, /Run \/code-review:setup/i);
});
