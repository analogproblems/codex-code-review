...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    const actions = [`/code-review:status ${job.id}`];
    if (job.status === "queued" || job.status === "running") {
      actions.push(`/code-review:cancel ${job.id}`);
    }
    lines.push(
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.repository) {
    lines.push(`  Repository: ${job.repository}`);
  }
  if (job.prUrl) {
    lines.push(`  Pull request: ${job.prUrl}`);
  }
  if (job.commentUrl) {
    lines.push(`  Posted comment: ${job.commentUrl}`);
  }
  ...
  if ((job.status === "queued" || job.status === "running") && options.showCancelHint) {
    lines.push(`  Cancel: /code-review:cancel ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && options.showResultHint) {
    lines.push(`  Result: /code-review:result ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    lines.push("  Review changes: /code-review:review --wait");
    lines.push("  Stricter review: /code-review:adversarial-review --wait");
  }
...
export function renderSetupReport(report) {
  const lines = [
    ...
    `- GitHub: ${report.github.detail}`,
    `- PR review: ${report.pullRequestReviewReady ? "ready" : "needs attention"}`,
...
export function renderStoredJobResult(job, storedJob) {
  ...
  if (isStructuredReviewStoredResult(storedJob) && storedJob?.rendered) {
    ...
    if (!threadId) {
      return appendStoredReviewMetadata(output, job, storedJob);
    }
    return appendStoredReviewMetadata(`${output}\nCodex session ID: ${threadId}\nResume in Codex: ${resumeCommand}\n`, job, storedJob);
  }
  ...
  return appendStoredReviewMetadata(`${lines.join("\n").trimEnd()}\n`, job, storedJob);
}
...
function appendStoredReviewMetadata(output, job, storedJob) {
  ...
  if (metadata.prUrl) {
    lines.push(`Pull request: ${metadata.prUrl}`);
  }
  if (metadata.commentUrl) {
    lines.push(`Posted comment: ${metadata.commentUrl}`);
  }
  if (metadata.cleanupStatus) {
    lines.push(`Cleanup: ${metadata.cleanupStatus}`);
  }
  if (metadata.cleanupStatus === "needs-attention" && metadata.cleanup?.cleanupCommand) {
    lines.push("Cleanup command:", metadata.cleanup.cleanupCommand);
  }
  if (lines.length === 0) {
    return output;
  }
...
export function renderCancelReport(job) {
  ...
  if (job.cleanupStatus === "needs-attention" && job.cleanup?.cleanupCommand) {
    lines.push("- Temporary checkout cleanup needs attention:");
    for (const command of String(job.cleanup.cleanupCommand).split(/\r?\n/)) {
      lines.push(`  ${command}`);
    }
  } else if (job.cleanupStatus) {
    lines.push(`- Cleanup: ${job.cleanupStatus}`);
  }
  lines.push("- Check `/code-review:status` for the updated queue.");
